...
function loggedInWithSession(req, res, next) {
  ...
  if (req.loggedIn) {
    ...
    debug(
      '%s - %s - `loggedInWithSession` - user already logged in',
      req.id,
      req.originalUrl
    );
    return next();
  ...
  if (req.session.auth) {
    debug(
      '%s - %s - `loggedInWithSession` - found session`',
      req.id,
      req.originalUrl
    );
    req.username = req.session.username;
    ...
    debug(
      '%s - %s - `loggedInWithSession` - no session found',
      req.id,
      req.originalUrl
    );
    req.loggedIn = false;
...
async function loggedInWithJwt(req, res, next) {
  ...
  if (req.loggedIn) {
    debug(
      '%s - %s - `loggedInWithJwt` - user already logged in',
      req.id,
      req.originalUrl
    );
    return next();
  ...
  if (!token || (token && token[0].toLowerCase() !== 'bearer')) {
    debug(
      '%s - %s - `loggedInWithJwt` - invalid or missing token',
      req.id,
      req.originalUrl
    );
    req.loggedIn = false;
    ...
    try {
      ...
      const payload = await verifyJwt(
        token[1],
        req.app.get('JWT_SIGNING_CERTIFICATE')
      );
      if (!payload) {
...
function redirectToLoginIfNecessary(req, res, next) {
  debug(
    '%s - %s - `redirectToLoginIfNecessary` called',
    req.id,
    req.originalUrl
  );
  if (req.loggedIn) {
    debug(
      '%s - %s - `redirectToLoginIfNecessary` - logged in',
      req.id,
      req.originalUrl
    );
    next();
    ...
    debug(
      '%s - %s - `redirectToLoginIfNecessary` - redirecting to login',
      req.id,
      req.originalUrl
    );
    res.redirectToLogin();
...
async function loggedin(req, res, next) {
  ...
  if (req.isApiRequest) {
    ...
    if (!token || (token && token[0].toLowerCase() !== 'bearer')) {
      debug(
        '%s - %s - Invalid or missing token, redirecting to login',
        req.id,
        req.originalUrl
      );
  ...
  if (req.session.auth) {
    debug(
      '%s - %s - `loggedin` called, user has session, calling `next()`',
      req.id,
      req.originalUrl
    );
    return next();
  ...
  debug(
    '%s - %s - User not logged in, redirecting to login',
    req.id,
    req.originalUrl
  );
  res.redirectToLogin();
...
function authenticateCasUser(req, res, next) {
  cas.authenticate(
    req,
    res,
    (err, status, username, extended) => {
      username = username.trim();
      if (err) {
        debug(
          '%s - %s - Error authenticating user with CAS: %s',
          req.id,
          req.originalUrl,
          err
        );
        ...
        debug(
          `%s - %s - Authenticated user %s via CAS`,
          req.id,
          req.originalUrl,
          username
        );
        ...
        req.session.regenerate(() => {
          ...
          debug(
            `%s - %s - Regenerating session: ${JSON.stringify(
              req.session,
              null,
              2
            )}`,
  ...
  try {
    ...
    debug(
      req.user
        ? `${req.id} - ${
            req.originalUrl
          } - Retreived user record for ${username}: ${JSON.stringify(
            req.user,
            null,
            2
          )}`
        : `${req.id} - ${req.originalUrl} - No user record exists for ${username}`
    );
    if (req.session) {
    ...
    debug(
      '%s - %s - Error getting user from DB: %s',
      req.id,
      req.originalUrl,
      e
    );
...
async function getProxyTicket(req, res, next) {
  debug(
    '%s - %s - Getting proxy ticket for %s',
    req.id,
    req.originalUrl,
    req.username
  );
  try {
    req.PROXY_TICKET = await cas.getProxyTicketAsync(
      req.session.casAttributes.PGTIOU,
      process.env.PORTAL_SERVICE_NAME
    );
    debug(
      '%s - %s - Proxy ticket: %s',
      req.id,
    ...
    debug(
      '%s - %s - Error getting proxy ticket: %s',
      req.id,
      req.originalUrl,
      e
    );
...
async function provisionOrUpdateUser(req, res, next) {
  ...
  if (req.user || req.session.user) {
    ...
    const accessTokenStillValid = userHasAccessToken
      ? await validateAccessToken(
          user.oauth_access_token,
          new Date(user.oauth_valid_until).getTime()
        )
      : false;
    debug(
      '%s - %s - User %s exists in DB, access token valid? %s',
      req.id,
      req.originalUrl,
      user.username,
    ...
    if (accessTokenStillValid) {
      ...
      try {
        debug(
          '%s - %s - Getting new OAuth credentials for %s',
          req.id,
          req.originalUrl,
          user.username
        );
        ...
        debug(
          '%s - %s - Updating user record in DB with new OAuth credentials',
          req.id,
          req.originalUrl
        );
        req.user = req.session.user = await updateOAuthCredentialsForUser(
          user.username,
        ...
        debug(
          '%s - %s - Error getting new OAuth credentials: %s',
          req.id,
          req.originalUrl,
          e
        );
  ...
  try {
    debug(
      '%s - %s - User %s does not exist in DB, fetching OAuth credentials and creating record',
      req.id,
      req.originalUrl,
      req.username
    );
    ...
    if (req.isApiRequest) {
      debug(
        '%s - %s - This is an API request, getting bio',
        req.id,
        req.originalUrl
      );
      ...
      debug(
        '%s - %s - Not an API request; getting OAuth credentials and bio',
        req.id,
        req.originalUrl
      );
    ...
    debug(
      '%s - %s - Attempting to create user record in DB',
      req.id,
      req.originalUrl
    );
