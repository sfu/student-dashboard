...
export default async function (req, res, config) {
  ...
  const oauthCredentials = await loadUser(username, [
    'oauth_access_token',
    'oauth_refresh_token',
    'oauth_valid_until',
  ]);
  ...
  debug(
    'Attempting to make OAuthenticated Request to %s for user %s using token %s',
    config.url,
    username,
    access_token
  );
  ...
  if (!accessTokenValid) {
    ...
    if (!refresh_token || !access_token) {
      ...
      try {
        const proxyTicket = await cas.getProxyTicketAsync(
        ...
        debug(
          'Got OAuth credentials: %s',
          JSON.stringify(newCredentialsResponse.data)
        );

        if (
          newCredentialsResponse.data &&
          newCredentialsResponse.data.access_token
        ) {
  ...
  request.interceptors.request.use((config) => {
    config.headers['Authorization'] = `Bearer ${
      updateCredentials.access_token || access_token
    }`;
    return config;
  });
