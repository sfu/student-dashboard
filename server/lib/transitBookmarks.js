...
export const getBookmarksForUser = async (user_id, columns = ['id', 'stop', 'route', 'destination']) => {
  try {
...
export const getBookmarkById = async (id, columns = ['id', 'stop', 'route', 'destination']) => {
  try {
...
export const ownsBookmark = async (req, res, next) => {
  ...
  try {
    ...
    debug('%s - Error in `ownsBookmark` middleware for user %s: %s', req.id, username, e)
