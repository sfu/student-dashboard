...
const renderStops = (stops, isFetching) =>
  stops.map((stop, i) => {
    const { number, onStreet, atStreet, nextArrival } = stop;
    const time = !nextArrival
      ? 'Unknown'
      : nextArrival.ExpectedCountdown < 2
      ? 'Now'
      : `${nextArrival.ExpectedCountdown} min`;
    const tracker = (ev) => {
      ev.stopPropagation();
      ReactGA.event({
          ...
          <div className={styles.listItem}>
            <div className={styles.container}>
              <div className={styles.stopInfo}>
                <p>{`${transformTranslinkText(
                  onStreet
                )} / ${transformTranslinkText(atStreet)}`}</p>
                <p>{number}</p>
              </div>
              ...
              <div className={styles.linkContainer}>
                <img
                  src={CaretRight}
                  alt={`More information for stop ${number}`}
                  className={styles.linkCaret}
                />
              </div>
            </div>
          </div>
    ...
    <div className={styles.row}>
      ...
      <div className={styles.routeInfo}>
        <h1 className={styles.route}>
          {route} {transformTranslinkText(destination)}
        </h1>
...
export default connect((state) => ({ transit: state.transit }))(
  DashboardTransitBookmarkRow
);
