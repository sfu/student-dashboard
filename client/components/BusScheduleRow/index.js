...
import { addTransitBookmark, removeTransitBookmark } from 'actions/transit';
...
const arrivalTimes = (schedules) => {
  const sorted = schedules.sort(
    (a, b) => a.ExpectedCountdown > b.ExpectedCountdown
  );
  ...
  return sorted.slice(0, 3).map((item, i) => {
    ...
    return (
      <li key={i} className={styles.arrivalTime}>
        {i === 0 && ScheduleStatus !== '*' ? (
          <img
            alt="Real-time arrival estimate"
...
const BusScheduleRow = ({
  ...
  const bookmarkForStop = transitBookmarks.find((bookmark) => {
    const clone = {
      ...bookmark,
      destination: bookmark.destination.toLowerCase(),
    };
    delete clone.id;
    return isEqual(clone, {
      stop: stopNumber,
      route: schedules.RouteNo,
      destination: destination.toLowerCase(),
    });
  });

  const BookmarkIcon = bookmarkForStop
    ? BookmarkIconFilled
    : BookmarkIconHollow;
  return (
    <div className={styles.row}>
      ...
      <div className={styles.routeInfo}>
        ...
        <h2 className={styles.destination}>
          {transformTranslinkText(destination)}
        </h2>
      </div>
      ...
      <input
        ...
        title={
          bookmarkForStop
            ? `Remove from transit bookmarks`
            : `Add to transit bookmarks`
        }
        onClick={() => {
          if (bookmarkForStop) {
