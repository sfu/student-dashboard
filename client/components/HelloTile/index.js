...
export const _HelloTile = React.createClass({
  ...
  numberOfItemsOfType(data, type) {
    ...
    return data.filter(
      (i) =>
        new Date(i.start_at).getDay() === this.today &&
        type.indexOf(i.type) >= 0
    ).length;
  },
  ...
  youHave(data) {
    ...
    const strings = Object.keys(stats)
      .map((t) => {
        const qty = stats[t];
        if (qty === 0) {
          return null;
    ...
    switch (strings.length) {
      ...
      case 2:
        str = (
          <span>
            You have <b>{strings.join(' and ')}</b> today
          </span>
        );
        break;
      default:
        str = (
          <span>
            You have{' '}
            <b>
              {[strings.slice(0, -1).join(', '), strings.slice(-1)[0]].join(
                strings.length < 2 ? '' : ', and '
              )}
            </b>{' '}
            today.
          </span>
  ...
  render() {
    ...
    return (
      <div className={styles.helloTile}>
        ...
        <p className={styles.p}>
          {this.youHave(helloTileSchedule.scheduleForRangeInTerm)}
        </p>
...
export const HelloTile = connect(
  (state) => state,
  mapDispatchToProps
)(
  Relay.createContainer(_HelloTile, {
    initialVariables: {
