...
export const FETCH_SCHEDULE_FOR_BUS_STOP_START =
  'FETCH_SCHEDULE_FOR_BUS_STOP_START';
export const FETCH_SCHEDULE_FOR_BUS_STOP_SUCCESS =
  'FETCH_SCHEDULE_FOR_BUS_STOP_SUCCESS';
export const FETCH_SCHEDULE_FOR_BUS_STOP_ERROR =
  'FETCH_SCHEDULE_FOR_BUS_STOP_ERROR';
export const FETCH_SCHEDULE_FOR_BUS_STOP = 'FETCH_SCHEDULE_FOR_BUS_STOP';

export const FETCH_SCHEDULES_FOR_BOOKMARKS_START =
  'FETCH_SCHEDULES_FOR_BOOKMARKS_START';
export const FETCH_SCHEDULES_FOR_BOOKMARKS_SUCCESS =
  'FETCH_SCHEDULES_FOR_BOOKMARKS_SUCCESS';
export const FETCH_SCHEDULES_FOR_BOOKMARKS_ERROR =
...
export const SEARCH_FOR_STOP_UPDATE_FIELD_VALUE =
  ...
  return (dispatch, getState) => {
    ...
    return axios
      ...
      .then((response) => {
        ...
        const nextState = uniqBy(
          getState().transit.stops.concat(newStop),
          'StopNo'
        );
...
export const fetchStops = (coords, radius) => {
  return (dispatch, getState) => {
    ...
    return axios
      ...
      .then((response) => {
        const nextState = uniqBy(
          getState().transit.stops.concat(response.data),
          'StopNo'
        );
        dispatch(fetchStopsSuccess(nextState));
      })
...
export const fetchSchedulesForBusStop = (stop) => {
  return (dispatch) => {
    ...
    return axios
      ...
      .then((response) => {
        dispatch(
          fetchSchedulesForBusStopSuccess(normalizeTranslinkData(response.data))
        );
      })
      .catch((error) => {
...
export const fetchSchedulesForBusStopStart = () => ({
  type: FETCH_SCHEDULE_FOR_BUS_STOP_START,
});
